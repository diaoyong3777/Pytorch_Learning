"""
        ...
        这里有些问题，图像像素每个通道原本是-255到255的，toTensor自动进行了归一化，单纯的Norm无法保证都在0和1之间的
        Norm理解为(现实世界近似于正态分布)标准化：对每个通道的每个数据减去均值再除以标准差
        归一化是为了消除奇异值，及样本数据中与其他数据相比特别大或特别小的数据 这样可以加快训练速度
        归一化有利于预防梯度爆炸加速损失函数收敛。归一化在训练神经网络时候有作用，可以压缩值域，防止梯度爆炸。归一化用来消除量纲的影响
"""
